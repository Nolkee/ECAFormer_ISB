...
结论：锚定强度是干净的单变量权衡。R51 扫描 w 0.1/0.25，并引入**相对死区**
（`anchor_deadzone: 0.15`，w0.5 的力只作用于偏离目标 ±15% 之外；x1 可达域
仅 ±33%，故带宽必须 < (2-ratio)/ratio，代码在 init 强制校验）——目标同时拿到
r50a 的 PSNR 自由度和 r50c 的稳定性。
